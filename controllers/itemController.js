...
export const getAllItems = async (req, res) => {
    try {
        ...
        if(items.length < 1) return res.status(404).json('No item found yet')
...
export const createItem =  async (req, res) => {
    ...
    try {
        ...
        const newItem = await Item.create({
            title,
            description,
            image: req.file ? 'public/images/' + req.file.filename: 'public/images/default-image.png',
            price,
            category
        })
        if(!newItem) {
        ...
        return  res.status(201).json({message: 'Item has been created', createItem})
    } catch (error) {
        console.log(error)
        return res.status(500).json('Internal server error')
    }
